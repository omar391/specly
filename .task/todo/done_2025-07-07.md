...
- **Description**:
  - Create global database in `~/.specly/global.db` for workspace registry and global configs
  - Move workspace-specific data to `{workspace}/.specly/task.db` per workspace
...
- **Notes**: ✅ COMPLETED - Dual database architecture successfully implemented with:
  - Global DB: ~/.specly/global.db for workspaces, sessions, global tool flows, feedback steps, MCP mappings
  - Workspace DB: {workspace}/.specly/task.db for tasks, GitHub configs, remote interfaces (on-demand initialization)
...
- **Description**:
  ...
  - Document database locations: ~/.specly/global.db and {workspace}/.specly/task.db
...
- **Notes**: ✅ COMPLETED - Phase 3 architecture decisions comprehensively documented:
  ...
  - Documented dual database architecture: ~/.specly/global.db + {workspace}/.specly/task.db
...
## Task TP-030: Fix Start/Init Tool Flow Context (duplicate)
- **Title**: Update start/init tools to return workspace rules as feedback step
- **Description**:
  - Modify global-tool-flows-db-seed.json to add feedback steps for start/init tools
  - Create "workspace_context" composite feedback step containing standard + workspace rules + analytical framework
