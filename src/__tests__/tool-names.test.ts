...
describe('Tool Names', () => {
  describe('ToolNames Enum', () => {
    it('should define INIT tool', () => {
      expect(ToolNames.INIT).toBe('specly_init');
    });

    it('should define START tool', () => {
      expect(ToolNames.START).toBe('specly_start');
    });

    it('should define ADD tool', () => {
      expect(ToolNames.ADD).toBe('specly_add');
    });

    it('should define STATUS tool', () => {
      expect(ToolNames.STATUS).toBe('specly_status');
    });

    it('should define UPDATE tool', () => {
      expect(ToolNames.UPDATE).toBe('specly_update');
    });

    it('should define AUDIT tool', () => {
      expect(ToolNames.AUDIT).toBe('specly_audit');
    });

    it('should define FOCUS tool', () => {
      expect(ToolNames.FOCUS).toBe('specly_focus');
    });

    it('should define GITHUB tool', () => {
      expect(ToolNames.GITHUB).toBe('specly_github');
    });

    it('should define RULE_UPDATE tool', () => {
      expect(ToolNames.RULE_UPDATE).toBe('specly_rule_update');
    });

    it('should define REMOTE_INTERFACE tool', () => {
      expect(ToolNames.REMOTE_INTERFACE).toBe('specly_remote_interface');
    });

    it('should define UPDATE_RESOURCES tool', () => {
      expect(ToolNames.UPDATE_RESOURCES).toBe('specly_update_resources');
    });

    it('should define UPDATE_STEPS tool', () => {
      expect(ToolNames.UPDATE_STEPS).toBe('specly_update_steps');
    });

    it('should have consistent specly_ prefix', () => {
      const allValues = Object.values(ToolNames);
      for (const value of allValues) {
        expect(value).toMatch(/^specly_/);
      }
    });
  ...
  describe('TOOL_NAMES Array', () => {
    ...
    it('should allow filtering', () => {
      ...
      expect(updateTools).toContain('specly_update');
    });

    it('should allow mapping', () => {
      const upperCaseNames = TOOL_NAMES.map(name => name.toUpperCase());
      expect(upperCaseNames).toContain('SPECLY_INIT');
    });
  ...
  describe('isValidToolName', () => {
    it('should return true for INIT', () => {
      expect(isValidToolName('specly_init')).toBe(true);
    });

    it('should return true for START', () => {
      expect(isValidToolName('specly_start')).toBe(true);
    });

    it('should return true for ADD', () => {
      expect(isValidToolName('specly_add')).toBe(true);
    });

    it('should return true for STATUS', () => {
      expect(isValidToolName('specly_status')).toBe(true);
    });

    it('should return true for UPDATE', () => {
      expect(isValidToolName('specly_update')).toBe(true);
    });

    it('should return true for AUDIT', () => {
      expect(isValidToolName('specly_audit')).toBe(true);
    });

    it('should return true for FOCUS', () => {
      expect(isValidToolName('specly_focus')).toBe(true);
    });

    it('should return true for GITHUB', () => {
      expect(isValidToolName('specly_github')).toBe(true);
    });

    it('should return true for RULE_UPDATE', () => {
      expect(isValidToolName('specly_rule_update')).toBe(true);
    });

    it('should return true for REMOTE_INTERFACE', () => {
      expect(isValidToolName('specly_remote_interface')).toBe(true);
    });

    it('should return true for UPDATE_RESOURCES', () => {
      expect(isValidToolName('specly_update_resources')).toBe(true);
    });

    it('should return true for UPDATE_STEPS', () => {
      expect(isValidToolName('specly_update_steps')).toBe(true);
    });
    ...
    it('should be case-sensitive', () => {
      expect(isValidToolName('SPECLY_INIT')).toBe(false);
      expect(isValidToolName('Specly_Init')).toBe(false);
    });

    it('should not accept partial matches', () => {
      expect(isValidToolName('specly')).toBe(false);
      expect(isValidToolName('init')).toBe(false);
      expect(isValidToolName('specly_')).toBe(false);
    });

    it('should not accept similar but incorrect names', () => {
      expect(isValidToolName('specly_initializer')).toBe(false);
      expect(isValidToolName('specly-init')).toBe(false); // hyphen instead of underscore
    });
  ...
  describe('validateToolName', () => {
    it('should not throw for valid tool names', () => {
      expect(() => validateToolName('specly_init')).not.toThrow();
      expect(() => validateToolName('specly_start')).not.toThrow();
      expect(() => validateToolName('specly_add')).not.toThrow();
    });
    ...
    it('should list available tools in error message', () => {
      try {
        ...
        expect(error.message).toContain('specly_init');
        expect(error.message).toContain('specly_start');
        expect(error.message).toContain('specly_add');
      }
    });
  ...
  describe('Integration Scenarios', () => {
    ...
    it('should work in configuration objects', () => {
      ...
      expect(config.validateTool('specly_init')).toBe(true);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle tool name with newlines', () => {
      expect(isValidToolName('specly_init\n')).toBe(false);
    });

    it('should handle unicode lookalikes', () => {
      // Unicode underscore lookalike
      expect(isValidToolName('specly＿init')).toBe(false);
    });
