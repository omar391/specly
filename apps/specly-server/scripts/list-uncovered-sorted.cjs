...
const entries = Object.entries(cov).map(([path, entry]) => {
...
}).filter(x => x.stmtPct < 100 || x.fnPct < 100 || x.branchPct < 100).map(x=> ({...x, uncoveredCount: x.uncoveredLines.length})).sort((a,b)=> a.uncoveredCount - b.uncoveredCount || a.stmtPct - b.stmtPct);
